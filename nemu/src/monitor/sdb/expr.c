...
int get_mainoperator_pos(int p, int q)
{
  ...
  */
  ...
  for (int i = p; i <= q; i++)
  {
    ...
    if (tokens[i].type == TK_GT || tokens[i].type == TK_LT || tokens[i].type == TK_EQ || tokens[i].type == TK_NE || tokens[i].type == TK_GE || tokens[i].type == TK_LE)
    {
      if (presence <= 4)
      {
        pos = i;
...
word_t expr(char *e, bool *success)
{
  ...
  {
    if (tokens[i].type == '*' && (i == 0 || tokens[i - 1].type == TK_AND || tokens[i - 1].type == TK_BAND || tokens[i - 1].type == TK_BNOR || tokens[i - 1].type == TK_BOR || tokens[i - 1].type == TK_DEREF || tokens[i - 1].type == TK_EQ || tokens[i - 1].type == TK_GE || tokens[i - 1].type == TK_GT || tokens[i - 1].type == TK_LE || tokens[i - 1].type == TK_LT || tokens[i - 1].type == TK_NE || tokens[i - 1].type == TK_NOR || tokens[i - 1].type == TK_OR || tokens[i - 1].type == TK_SL || tokens[i - 1].type == TK_SR || tokens[i - 1].type == '+' || tokens[i - 1].type == '-' || tokens[i - 1].type == '*' || tokens[i - 1].type == '/'))
    {
