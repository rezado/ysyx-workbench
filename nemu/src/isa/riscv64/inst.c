...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + src2, 1));  // 无符号数自动零扩展
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, R(dest) = SEXT(BITS(src1, 31, 0) << src2, 32));
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(ECALL, s->pc); if (s->dnpc == 0x800005a0) printf("reach mtvec pc\n"));
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, assert(0); printf("into mret\n"); s->dnpc = cpu.csr[MEPC]; word_t MIE = BITS(cpu.csr[MSTATUS], 8, 8); if (MIE == 1) cpu.csr[MSTATUS] |= 0x10000000; else cpu.csr[MSTATUS] &= ~0x10000000; cpu.csr[MSTATUS] |= 0x8);
