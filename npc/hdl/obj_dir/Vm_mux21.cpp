...
QData Vm_mux21___024root___change_request(Vm_mux21___024root* vlSelf);
#ifdef VL_DEBUG
...
static void _eval_initial_loop(Vm_mux21__Syms* __restrict vlSymsp) {
    ...
    do {
        ...
        if (VL_UNLIKELY(++__VclockLoop > 100)) {
            ...
            Verilated::debug(1);
            __Vchange = Vm_mux21___024root___change_request(&(vlSymsp->TOP));
            Verilated::debug(__Vsaved_debug);
            VL_FATAL_MT("m_mux21.v", 1, "",
...
static void traceInit(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    tracep->scopeEscape(' ');
    Vm_mux21___024root__traceInitTop(vlSelf, tracep);
    tracep->scopeEscape('.');
}

void Vm_mux21___024root__traceRegister(Vm_mux21___024root* vlSelf, VerilatedVcd* tracep);

void Vm_mux21::trace(VerilatedVcdC* tfp, int, int) {
    tfp->spTrace()->addInitCb(&traceInit, &(vlSymsp->TOP));
    Vm_mux21___024root__traceRegister(&(vlSymsp->TOP), tfp->spTrace());
}
